...
func (r *Router) setupCarRoutes(router *mux.Router) {
	...
	router.HandleFunc("/cars", r.CarHandler.GetAllCars).Methods("GET")

	// GET /cars/{id} - Retrieve a specific car by its UUID
	// Path parameter: UUID of the car
	router.HandleFunc("/cars/{id}", r.CarHandler.GetCarByID).Methods("GET")
	...
	router.Handle("/cars", middleware.ImageUploadMiddleware(http.HandlerFunc(r.CarHandler.CreateCar))).Methods("POST")
	...
	router.Handle("/cars/{id}", middleware.ImageUploadMiddleware(http.HandlerFunc(r.CarHandler.UpdateCar))).Methods("PUT")

	// DELETE /cars/{id} - Delete a car by its UUID
	// Path parameter: UUID of the car to delete
	router.HandleFunc("/cars/{id}", r.CarHandler.DeleteCar).Methods("DELETE")
}
