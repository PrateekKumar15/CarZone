...
func (r *Router) setupAuthRoutes(router *mux.Router) {
	...
	router.HandleFunc("/auth/register", r.AuthHandler.RegisterHandler).Methods("POST")

	// POST /auth/login - Authenticate user and receive access token
	router.HandleFunc("/auth/login", r.AuthHandler.LoginHandler).Methods("POST")

	// GET /auth/logout - Logout user (invalidate session)
	router.HandleFunc("/auth/logout", r.AuthHandler.LogoutHandler).Methods("GET")
}
